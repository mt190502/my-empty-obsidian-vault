...
class AutoIndex:
    ...
    def generateindex(self, indexdir, excludedir, listcount, newnotebutton, reverse, wikilinktags):
        ...
        if newnotebutton.split(":")[0].lower() == "true":
            x = ''.join(random.choice(string.ascii_uppercase + string.ascii_lowercase + string.digits) for _ in range(5))
            if len(newnotebutton.split(':')) >= 2:
                y = newnotebutton.split(":")[1]
            else:
                y = "Note"

            tmp.append(f"- [[{indexdir}/{y} {x}.md| + New Note +]]\n")
    ...
    def savefullyindex(self, generatedindex, targetfile):
        with open(f"{targetfile}.md", "r") as f:
            ...
            indextitle = re.findall("(?<=~>\s)[\d\w\s]+(?=\s<~)", g)[0]
...
links: {links}\n\
----\n\
<p align=\"center\" style=\"font-size: 25; font-weight: bold;\"> ~> {indextitle} <~ </p>\n\n\
----\n\
\n\
{joinedlist}\
\n\
